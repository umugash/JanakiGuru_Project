...
export default function CheckoutPage() {
  ...
  // Each cart item has its own price (variant price already set correctly)
  const getItemPrice = (item: any) => Number(item.website_price) || Number(item.price) || 0;
  const total = cart.reduce((sum, item) => sum + getItemPrice(item) * (item.quantity || 1), 0);

  // Unique key per line item (product + variant)
  ...
  function increaseQty(item: any) {
    ...
    setCart(prev => prev.map(p => lineKey(p) === key ? { ...p, quantity: (p.quantity || 1) + 1 } : p));
  }

  function decreaseQty(item: any) {
    const key = lineKey(item);
    setCart(prev => prev.map(p => lineKey(p) === key ? { ...p, quantity: (p.quantity || 1) - 1 } : p).filter(p => p.quantity > 0));
  }

  function removeItem(item: any) {
    const key = lineKey(item);
    setCart(prev => prev.filter(p => lineKey(p) !== key));
  }
  ...
  const sendTelegram = async (name: string, phone: string, address: string, items: any[], orderAmt: number) => {
    // Each variant shows as separate line
    const itemsList = items.map((item: any) => {
      const label = item.variantLabel ? ` (${item.variantLabel})` : "";
      return `  • ${item.name}${label} × ${item.quantity} = ₹${getItemPrice(item) * item.quantity}`;
    }).join("\n");
  ...
  const handlePlaceOrder = async () => {
    ...
    // Build order items — each variant is a separate line for admin panel
    const orderItems = capturedCart.map(item => ({
  ...
  if (success) {
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#fff5f5,#fff)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px", textAlign: "center", fontFamily: "system-ui, sans-serif" }}>
        <style>{`
        ...
        <div className="fade-up-3" style={{ background: "#fff", border: "2px solid #fee2e2", borderRadius: 18, padding: "18px 28px", marginBottom: 24, width: "100%", maxWidth: 340, boxShadow: "0 4px 20px rgba(220,38,38,0.1)" }}>
        ...
        <a href="/" style={{ textDecoration: "none", width: "100%", maxWidth: 340 }}>
          <div className="fade-up-4 pulse-btn" style={{ background: "linear-gradient(135deg,#ef4444,#b91c1c)", color: "#fff", borderRadius: 16, padding: "14px 28px", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>← Continue Shopping</div>
        </a>
      </div>
    );
  }
  ...
  if (cart.length === 0) {
    return (
      <div style={{ minHeight: "100vh", background: "#fff5f5", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "system-ui, sans-serif", padding: 32, textAlign: "center" }}>
        ...
        <a href="/" style={{ background: "linear-gradient(135deg,#ef4444,#b91c1c)", color: "#fff", borderRadius: 14, padding: "12px 28px", fontWeight: 700, fontSize: 15, textDecoration: "none" }}>← Browse Products</a>
      </div>
    );
  }
  ...
  return (
    <div style={{ fontFamily: "system-ui,sans-serif", minHeight: "100vh", background: "#fff5f5", paddingBottom: 100 }}>
      <div style={{ background: "linear-gradient(135deg,#ef4444,#b91c1c)", padding: "16px 16px 36px", color: "#fff", position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 4 }}>
          <a href="/" style={{ textDecoration: "none" }}>
            <div style={{ background: "rgba(255,255,255,0.2)", border: "1.5px solid rgba(255,255,255,0.4)", color: "#fff", borderRadius: 10, padding: "6px 12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>← Back</div>
          </a>
          <div>
        ...
        <div style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 10, padding: "6px 14px", display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700, color: "#fff", marginTop: 8 }}>
      ...
      {/* ORDER SUMMARY — each variant is a separate line */}
      <div style={{ margin: "32px 14px 0", background: "#fff", borderRadius: 18, padding: 16, boxShadow: "0 2px 16px rgba(220,38,38,0.07)", border: "1.5px solid #fee2e2" }}>
        ...
        {cart.map((item, idx) => (
          <div key={`${lineKey(item)}-${idx}`} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px dashed #fee2e2" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                {item.variantLabel && (
                  <div style={{ fontSize: 10, color: "#7c3aed", fontWeight: 700, marginTop: 1 }}>{item.variantLabel}</div>
                )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", background: "#fff1f1", borderRadius: 8, border: "1.5px solid #fca5a5", overflow: "hidden" }}>
                <button onClick={() => decreaseQty(item)} style={{ background: "none", border: "none", color: "#dc2626", fontSize: 16, fontWeight: 800, width: 28, height: 28, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#dc2626", minWidth: 20, textAlign: "center" }}>{item.quantity}</span>
                <button onClick={() => increaseQty(item)} style={{ background: "none", border: "none", color: "#dc2626", fontSize: 16, fontWeight: 800, width: 28, height: 28, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
              </div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#dc2626", minWidth: 46, textAlign: "right" }}>₹{getItemPrice(item) * item.quantity}</div>
              <button onClick={() => removeItem(item)} style={{ background: "#fff5f5", border: "1px solid #fca5a5", color: "#dc2626", borderRadius: 6, width: 26, height: 26, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✕</button>
            </div>
          </div>
        ))}
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <div style={{ flex: 1, background: "#fff1f1", borderRadius: 10, padding: "8px 12px", fontSize: 12, fontWeight: 600, color: "#dc2626", border: "1.5px solid #fca5a5" }}>🚚 Free delivery · Within 10 km</div>
          <div style={{ background: "#fffbeb", borderRadius: 10, padding: "8px 12px", fontSize: 12, fontWeight: 700, color: "#f59e0b", border: "1.5px solid #fde68a" }}>⚡ Same Day</div>
        </div>
      </div>
      ...
      <div style={{ margin: "14px 14px 0", background: "#fff", borderRadius: 18, padding: 16, boxShadow: "0 2px 16px rgba(220,38,38,0.07)", border: "1.5px solid #fee2e2" }}>
        ...
        {[
          ...
          <div key={field.key} style={{ marginBottom: 12 }}>
            ...
            <input type={field.type} placeholder={field.placeholder} value={(form as any)[field.key]}
              ...
              style={{ width: "100%", border: `2px solid ${(errors as any)[field.key] ? "#f87171" : "#fee2e2"}`, borderRadius: 12, padding: "10px 14px", fontSize: 14, fontFamily: "system-ui,sans-serif", color: "#1a1a1a", background: "#fffafa", outline: "none", boxSizing: "border-box" as const }} />
        ...
        <div>
          ...
          <textarea rows={3} placeholder="House no, street, area, landmark..." value={form.address}
            onChange={e => { setForm({ ...form, address: e.target.value }); setErrors({ ...errors, address: "" }); }}
            style={{ width: "100%", border: `2px solid ${errors.address ? "#f87171" : "#fee2e2"}`, borderRadius: 12, padding: "10px 14px", fontSize: 14, fontFamily: "system-ui,sans-serif", color: "#1a1a1a", background: "#fffafa", outline: "none", resize: "none", boxSizing: "border-box" as const }} />
      ...
      <div style={{ position: "fixed", bottom: 16, left: 14, right: 14 }}>
        <button onClick={handlePlaceOrder} disabled={loading} style={{ width: "100%", background: loading ? "#f87171" : "linear-gradient(135deg,#ef4444,#b91c1c)", color: "#fff", border: "none", borderRadius: 16, padding: 15, fontSize: 16, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "system-ui,sans-serif" }}>
